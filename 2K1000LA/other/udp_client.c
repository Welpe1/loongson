...
int main(int argc,char **argv)
{
    ...
    client_fd=socket(AF_INET,SOCK_DGRAM,0);    //创建套接字
    ...
    while(1)
    {
        ...
        {
            // int sendlen=send(client_fd,sendbuf,strlen(sendbuf),0);       //用connect函数时使用send

            int sendlen=sendto(client_fd,sendbuf,sizeof(sendbuf),0,(struct sockaddr *)&socket_server_addr,sizeof(struct sockaddr));     //不用connect函数时使用这个
